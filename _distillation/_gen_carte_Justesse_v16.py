...
NAME = "major_11_Justesse_v16_ciseaux_poses"
SEED = 777122

# XI · JUSTESSE — v15 validee, mais apres plusieurs generations le CISEAU tenu en main est
# presque toujours incoherent (doigts dans les anneaux, fil qui passe mal). Diagnostic :
# main + petit objet a mecanique fonctionnelle = point faible universel de ces modeles (meme
# famille de probleme que l'archer de v8-v11). Solution retenue (meme logique que le pivot
# v12 : retirer le probleme plutot que le decrire) : les ciseaux sont POSES OUVERTS sur le
# metier, a plat, enjambant le fil exactement au point de coupe. La femme ne tient plus que
# le fil — geste que le modele reussit bien. Symbolique renforcee : les ciseaux sont deja
# en place sur le point exact, il ne manque que le geste.
...
"the hand of a calm, precise woman holds the lower end of this red thread gently "
"between her fingers, a simple easy grip on a single strand — one hand only, holding "
"nothing else, no scissors in her hands at all. A short distance along the thread, the "
...
"only the thread. In the foreground, on the "
